...
for index in TrainFile.index:
    RawWordInThisLine = TrainFile.content[index].replace('，','').replace('。','').replace('！','').replace('\[a-z]*','')\
...
train_x,test_x,train_y,test_y = cross_validation.train_test_split(X, Y,test_size = 0.1,random_state = 66)
train_xs,test_xs,train_ys,test_ys = cross_validation.train_test_split(X, Ys,test_size = 0.1,random_state = 66)
